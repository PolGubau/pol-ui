...
[Flowbite React](https://github.com/polgubau/pol-ui) is a free and open-source UI component library based on the core Flowbite components and built with React components and interactivity handling.
...
```bash
npm i pol-ui # or yarn add flowbite pol-ui
```

2. Add the Flowbite plugin to `tailwind.config.js`, and include content from `pol-ui`:

```javascript
module.exports = {
  content: [
    ...,
    'node_modules/pol-ui/**/*.{js,jsx,ts,tsx}'
  ],
...
How you use Flowbite React depends on your project setup. In general, you can just import the components you want to use from `pol-ui` and use them in a React `.jsx` file:
