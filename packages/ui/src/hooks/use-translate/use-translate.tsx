...
const useTranslate = () => {
  ...
  const changeLanguage = (locale: Locale) => {
    ...
    if (!selectedLanguage) {
      console.warn(
        `😯 The language "${locale}" is not available in allLanguages, please add it`
      )
    }
  ...
  const getTranslationsByMultipleLanguages = (
    locales: Locale[]
  ): Translation => {
  ...
  const translate = (
    ...
    const fallbackLanguages =
      allLanguages.find((l) => l.locale === languageToTranslate)?.basedOn || []

    const languagesToTry = [languageToTranslate, ...fallbackLanguages]
    const languageTranslations =
      getTranslationsByMultipleLanguages(languagesToTry) ?? {}
    const keyExists = Object.keys(languageTranslations).find(
  ...
  const languagesGrouped = (
    langs: Language[],
    forceCountry: boolean = false
  ) => {
    ...
    const allLangs = forceCountry
      ? langs.filter((l) => l.country !== undefined)
      : langs

    const languagesGrouped = allLangs.reduce(
      (acc: Record<string, Language[]>, lang) => {
