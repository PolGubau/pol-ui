...
function useLongPress({ delay, onLongPress, onClick, onCancel, onFinish, onStart }: UseLongPressProps) {
  const timerRef = useRef<ReturnType<typeof setTimeout>>();
  const pressTriggeredRef = useRef<boolean>(false);
  const pressInitiatedRef = useRef<boolean>(false);

  const start = useCallback(
    (event: React.MouseEvent<unknown> | React.TouchEvent<unknown> | React.KeyboardEvent<unknown>) => {
