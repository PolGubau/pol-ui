...
const generateDocs = () => {
  ...
  const filteredComponents = components.filter(
    (componentPath) =>
      !(componentPath.includes("test") || componentPath.includes("stories") || componentPath.includes("spec")),
  );
  ...
  for (const componentPath of filteredComponents) {
    ...
    try {
      ...
      console.error(`Error al procesar el componente ${componentName}: ${error.message}`);
    }
  }
