...
import { type OverrideProps, cn, mergeDeep } from "../../helpers";
  ...
  const getPlaceholderValue = () => {
    if (typeof placeholder === "string") {
      ...
      if (placeholder.length > 0) {
        console.error("⚠️ Length of the placeholder should be equal to the number of inputs.");
      }
    }
    return undefined;
  };
        ...
        return (
          <React.Fragment key={index}>
            <input
              ...
              size={1}
              className={cn(theme.input, isSelected ? theme.selected : theme.unselected, inputsClassName)}
              type="text"
              inputMode={isInputNum ? "numeric" : "text"}
              onInput={handleInputChange}
              {...props}
            />
            {index < length - 1 && (typeof renderSeparator === "function" ? renderSeparator(index) : renderSeparator)}
          </React.Fragment>
        );
      })}
    </div>
