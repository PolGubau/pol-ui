      ...
      {isVisible && (
        <div className={cn(theme.floating.base)}>
          <svg
            ...
            className={cn(theme.floating.svg, theme.check.color[color], "stroke-current")}
          >
...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  (
    { checked = false, className, label, color = ColorsEnum.primary, theme: customTheme = {}, onChange, ...props },
    ref,
  ) => {
    const theme = mergeDeep(getTheme().checkbox, customTheme);
    ...
    return (
      <div className="inline-flex items-center gap-3 relative ">
        <input
          ...
          className={cn(theme.base, theme.before, theme.color[color], className)}
          ref={ref}
          type="checkbox"
        />
        ...
        <>
          ...
          <AnimatedIntermediateIcon isVisible={state.isIntermediate ?? false} color={color} />
        </>
