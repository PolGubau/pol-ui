...
export const isDateEqual = (date: Date, selectedDate: Date): boolean => {
  ...
  const currentDate = new Date(normalizedDate.getFullYear(), normalizedDate.getMonth(), normalizedDate.getDate());
...
export const getWeekDays = (lang = "en-US", weekStart: WeekStart = WeekStart.Monday): string[] => {
