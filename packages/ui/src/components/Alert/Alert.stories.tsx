...
AlertWithIcons.args = {
	...
	children: (
		<>
			Our recommendation is to use a descriptible icon with the content so the
			user will know faster what the context or the alert is, examples could be
			a LightBulb, a question mark, a info symbol...
		</>
	),
};
...
AlertDismissible.args = {
	...
	children: (
		<>
			Yeyy, that's a success alert! How the component has 'onDismiss' as a prop,
			you can trigger an action when clicking the cross.
		</>
	),
};
...
AlertRounded.args = {
	...
	children: (
		<>
			Warning alert! This alert is trying to advice you, be carefully that this
			actions is not undonable.
		</>
	),
};
...
Alertbordered.args = {
	...
	children: (
		<>
			Easier to see, right? Use this border when you have complexe contrast to
			increase the Alert visual importance
		</>
	),
};
	...
	additionalContent: (
		<>
			<div className="mb-4 mt-2 text-sm text-info-700 dark:text-info-800">
				This is the component comming from additionalContent as a prop, could be
				used when you need to present a large text that is better be cutted due
				to the icon.
			</div>
...
AlertWithAllOptions.args = {
	...
	children: (
		<>
			Error Alert! Example: If you continue all data stored here will be deleted
