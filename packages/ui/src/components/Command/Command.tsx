...
const defaultFilter: CommandProps["filter"] = (
  value: string,
  search: string,
  keywords: any
  ...
  (props, forwardedRef) => {
    ...
    function getSelectedItem() {
      return listInnerRef.current?.querySelector(
        `${SELECTORS.ITEM_SELECTOR}[aria-selected="true"]`
      )
    }

    function getValidItems() {
      return Array.from(
        listInnerRef.current?.querySelectorAll(SELECTORS.VALID_ITEM_SELECTOR) ??
          []
      )
    }
    ...
    function updateSelectedByItem(change: 1 | -1) {
      ...
      if (newSelected)
        store.setState(
          "value",
          newSelected.getAttribute(SELECTORS.VALUE_ATTR) ?? ""
        )
    }
