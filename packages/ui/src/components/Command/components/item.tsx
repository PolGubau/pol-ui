 ...
 */
export const CommandItem = React.forwardRef<HTMLButtonElement, ItemProps>(
  (props, forwardedRef) => {
    ...
    const forceMountBool =
      propsRef.current.forceMount ?? groupContext?.forceMount ?? false
    ...
    const selected = useCmdk(
      (state) => state.value && state.value === valueFromHook.current
    )
    ...
    React.useEffect(() => {
      ...
      return () => {
        element.removeEventListener(SELECTORS.SELECT_EVENT, onSelectValue)
      }
