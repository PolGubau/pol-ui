...
import type { PhoneNumber, usePhoneOptions } from "./types";
...
export const getCountry = (countryCode: CountryCode): Country | undefined => {
  return countries.find(([iso]) => iso === countryCode);
};
export const getCountryByName = (n: CountryName): Country | undefined => {
  return countries.find(([_, name]) => name === n);
};

export const getRawValue = (value: PhoneNumber | string): string => {
...
export const getFormattedNumber = (rawValue: any, pattern?: string) => {
  ...
  const p: string = pattern || getMetadata(rawValue)?.[3] || "";

  return displayFormat(cleanInput(rawValue, p.replaceAll(/\d/g, ".")).join(""));
};
...
export const getDefaultISO2Code = (): CountryCode => {
  /** Returns the default ISO2 code, based on the user's timezone */
  return (timezones[Intl.DateTimeFormat().resolvedOptions().timeZone as keyof typeof timezones] ||
...
export const parsePhoneNumber = (
  ...
  const countryCode = countryCodeMatch.length > 0 ? Number.parseInt(countryCodeMatch[0]) : null;
...
export const usePhone = ({
  ...
  const countriesOnly = useMemo(() => {
    ...
    return countries.filter(([iso, _1, dial]) => {
      return (
        (allowList.includes(iso) || allowList.includes(dial)) &&
        !excludeCountries.includes(iso) &&
        !excludeCountries.includes(dial)
      );
    });
  ...
  const countriesList = useMemo(() => {
    const filteredCountries = countriesOnly.filter(([_1, name, dial, mask]) => {
      ...
      //
      const localized = String(countryNames[name as keyof typeof countryNames] || "").toLowerCase();
