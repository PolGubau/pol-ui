import { isDeepKey } from "../isDeepKey/isDeepKey";
import { toPath } from "../toPath/toPath";
import type { Get } from "./get.types";
 ...
 */
export function toKey(value: number): string | symbol {
...
export function get(
  ...
  switch (typeof innerPath) {
    case "string": {
      const result = object[innerPath];

      if (result === undefined) {
        if (isDeepKey(innerPath)) {
          return get(object, toPath(innerPath), defaultValue);
        }
        return defaultValue;
      }
    ...
    case "number":
    case "symbol": {
      if (typeof innerPath === "number") {
        innerPath = toKey(innerPath);
      }
    ...
    default: {
      if (Array.isArray(innerPath)) {
        return getWithPath(object, innerPath, defaultValue);
      }
