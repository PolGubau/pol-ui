...
import { DayName, type RangeMonths, getDaysOfRangeMonth, isDayBefore } from "./utils";
...
export function useDateRange(rangeMonths: Date[], options: UseDateRangeOptions = {}): UseDateRangeReturn {
  ...
  const [range, setRange] = useState<DateRange>([options?.range?.[0] || null, options?.range?.[1] || null]);

  const { months, daysOfWeek } = getDaysOfRangeMonth(currentMonths, range[0], range[1], startOfWeek);
