...
export function setMonth(date: Date, month: number): Date {
  ...
  if (month < 0 || month > 11) {
    throw new RangeError(
      `Month must be number between 0 and 11, but got ${month}`
    )
  }
...
export function getDaysOfMonth(
  ...
  const daysOfWeek = DaysOfTheWeek.slice(dayOffset).concat(
    DaysOfTheWeek.slice(0, dayOffset)
  )
