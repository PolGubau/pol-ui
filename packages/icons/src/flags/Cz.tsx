import * as React from "react";
function SvgCz(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      width="1em"
      height="1em"
      viewBox="0 0 63 50"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M48.06 3.031H14.063c-3.739 0-5.073.443-6.434 1.171A8.24 8.24 0 004.202 7.63c-.728 1.361-1.17 2.695-1.17 6.434v21.874c0 3.739.442 5.073 1.17 6.434A8.24 8.24 0 007.63 45.8c1.361.728 2.695 1.17 6.434 1.17h33.995c3.739 0 5.073-.442 6.434-1.17a8.24 8.24 0 003.428-3.428c.728-1.361 1.17-2.695 1.17-6.434V14.064c0-3.739-.442-5.073-1.17-6.434a8.24 8.24 0 00-3.428-3.428c-1.36-.728-2.695-1.17-6.434-1.17z"
        fill="#fff"
        stroke="#fff"
        strokeWidth={3.03}
      />
      <path
        d="M57.577 25v10.937c0 3.31-.344 4.51-.991 5.72a6.746 6.746 0 01-2.807 2.806c-1.21.647-2.41.992-5.72.992H14.065c-3.31 0-4.51-.345-5.719-.992a6.745 6.745 0 01-2.807-2.806c-.647-1.21-.991-2.41-.991-5.72V25h53.03z"
        fill="#F73030"
      />
      <path
        d="M7.022 43.547a6.777 6.777 0 01-1.484-1.888c-.647-1.21-.991-2.41-.991-5.72V14.065c0-3.31.344-4.51.992-5.72a6.777 6.777 0 011.483-1.888l24.04 18.545-24.04 18.545z"
        fill="#1D80AE"
      />
    </svg>
  );
}
export default SvgCz;
